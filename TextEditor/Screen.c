...
void print_buffer(int y_bottom, int x_left){
    ...
    erase();
    y_iterator = y_node_find(min(y_bottom - y_size, size_of_y_list+1 - y_size));
    int x_curs = 0;
    int y_curs = 0;
    move(y_curs,x_curs);
...
/**
int check_boundary(int bottom, int screen_size, int length){
    if ((bottom - screen_size) < length+1 - screen_size){
        return bottom - screen_size;
    }
    else{
        return length+1 - screen_size;
    }
}
**/